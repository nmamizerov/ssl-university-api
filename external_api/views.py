...
class ExternalAuthAttemptViewSet(viewsets.ModelViewSet):
    ...
    def auto_registration(self, request):
        ...
        return Response(
            {"code": authorized_user.temporary_code}, status=status.HTTP_200_OK
        )
...
class ExternalSimulatorsViewSet(viewsets.ModelViewSet):
    ...
    def set_paid(self, request, *args, **kwargs):
        ...
        if "user_id" in request.data:
            simulator_user = get_object_or_404(
                SimulatorUser, simulator=simulator, user__id=request.data["user_id"]
            )
        elif "user_email" in request.data:
            simulator_user = get_object_or_404(
                SimulatorUser,
                simulator=simulator,
                user__email=request.data["user_email"],
            )
        else:
            return Response(
                {
        ...
        if "set_paid" in request.data:
            try:
                ...
                return Response(
                    {"set_paid": "Неверный формат, должен быть тип boolean!"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
                {"set_paid": "В запросе отсутствует необходимое поле!"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        return Response(
            {"is_paid": simulator_user.simulator_paid}, status=status.HTTP_200_OK
        )
