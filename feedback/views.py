...
class GetFeedback(APIView):
    ...
    def put(self, request):
        ...
        if tg_user is not None and tg_user.active: 
            message = f"""Вы получили комментарий на ваш ответ в упражнении по обратной связи: 
<b>{u_f.situation.text}</b>
<i>{u_f.situation.phrase}</i>
формула: {u_f.structure}
...
<a href=\"https://skillslab.center/feedback/{request.data.get("seq")}\">Перейти к ответу</a>"""
    ...
    def get(self, request):
        ...
        book = bool(request.GET.get('book', False))
        if not book:
        ...
        if u_f is None or u_f.structure is None:
            u_f = FeedbackSituationUser.objects.create(
                situation = situation, 
                user = request.user
            )
            u_f.add_structure()
        if int(request.GET.get('reset', 0)) == 1: 
            u_f.add_structure()
