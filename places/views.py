...
from backend.application_viewset import (
    AdminApplicationViewSet,
    ApplicationReadOnlyViewSet,
)
...
class PlaceViewSet(ApplicationReadOnlyViewSet):
    ...
    def comment(self, request, *args, **kwargs):
        if (
            not "place" in request.data
            or not "comment" in request.data
            or not "answer_id" in request.data
        ):
            ...
            if p_place["id"] == place.id:
                for idx_cp, commented_answer in enumerate(p_place["commented_answers"]):
                    if commented_answer["id"] == request.data["answer_id"]:
                        ...
                        page_user.places[idx_p]["commented_answers"][idx_cp][
                            "comment"
...
<a href=\"https://skillslab.center/blogs/{commented_place.place.page.lesson.slug}\">Перейти к уроку</a>"""
            tgSend.objects.create(tg=tg_user, message=message)
        ...
        if place.comment_number > len(p_user.commented_answers):
            ...
            page_user.places[page_user_place_idx]["commented_answers"].append(
                answer_to_comment
            )
        else:
            page_user.places[page_user_place_idx]["complete"] = True
    ...
    def complete_place(self, place, request):
        ...
        page_user = UserPageProgress.objects.filter(
            page=place.page, user=request.user
        ).first()
        ...
        try:
            ...
            raise NotImplementedError(
                "Class `{}` does not implement `{}`".format(
                    Place.__class__.__name__, place.type
                )
            )
        ...
        try:
            if place.type in ("question", "questionuserchoice"):
                place_action(
                    place, request.user, page_user, answers=request.data["answers"]
                )
            elif place.type in (
                "openquestion",
                "openquestionexpert",
                ...
                place_action(
                    ...
                    request.user,
                    page_user,
                    user_answer_min=request.data["user_answer_min"],
                    user_answer_max=request.data["user_answer_max"],
                )
            else:
        ...
        next_place_data, next_page_data, next_lesson_data = place.finish_complete(
            request.user, simulator, page_user
        )
        return Response(
            {
                "place": next_place_data,
                "page": next_page_data,
                "lesson": next_lesson_data,
            }
        )
